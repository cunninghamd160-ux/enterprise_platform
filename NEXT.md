...
| **Observability** (ADR-0006) | OpenTelemetry logs, metrics and traces; console by default, OTLP push by environment; six required log fields; counters for requests, job runs and cache hits | A Prometheus `/metrics` endpoint: the OTel Prometheus exporter (`PrometheusMetricReader`) exposes the same counters for scraping with no app change — the first Prometheus in the environment, and ADR-0006 should say metrics leave by push (OTLP) or pull (`/metrics`) behind one seam. Grafana / Tempo / Loki compose profile the first time someone needs to look at a trace rather than grep a log. Swap the deprecated `LoggingHandler` for `opentelemetry-instrumentation-logging` |
...
| **Scheduled jobs** (the second consumption model) | `run_job()` wraps an `async def main()`: loads the manifest, builds and validates the declared connections, sets the principal to `job:<name>` and a request id per run, opens one `job.run` trace, audits every query, writes `job.completed{status, duration_ms}` and the `insights.job.runs{status}` counter, and turns the outcome into exit code 0 or 1. Compose runs a job once under the `jobs` profile | The schedule itself is deliberately absent — nothing in the repo triggers a job. Plan: `[job] schedule = "<cron>"` in `platform.toml` (`manifest-valid` learns it) that the deploy path turns into the runtime's own scheduler object — a Kubernetes `CronJob` with `concurrencyPolicy: Forbid` when Kubernetes arrives, host cron over `docker compose run --rm` until then — so the SDK never becomes a scheduler. The contract the scheduler consumes already exists: the exit code and the `job.completed` record. Add alerts on `insights.job.runs{status="failed"}` and on a missed window, retries at the scheduler with an idempotent `main()`. Trigger: the first job that must run unattended, which is the first environment with a scheduler — the same trigger as real Kubernetes |
| **Web apps and frontends** | Vite + React scaffold served by the app under `/`, API under `/api` | `index.html` fallback for client-side routes when the first app needs them; label static requests `static` in the request counter; path-filter the frontend CI job to `frontend/**` when CI time bites |
...
- The frontend mount is not an `APIRoute`: `check_routes` ignores it and no marker applies, so the
  page shell is public and only `/api` is protected (ADR-0003, ADR-0005).
...
- Owned databases have two schema paths on purpose: models on the SQLite fixture, migrations on
  Postgres, so a unit test cannot prove a migration and the `integration` marker exists (ADR-0004,
  the owned store being a per-tenant resource).
