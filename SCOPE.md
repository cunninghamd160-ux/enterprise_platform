...
|---|----------|-------------------|-----------------|
| 1 | **Thin SDK in a `uv` workspace monorepo.** Not a service, not a framework. Apps depend on the SDK by workspace path and are always on HEAD. | 2–3 engineers can't operate a control plane. The upgrade story is *atomic*: an SDK change and the fixes to every dependent land in one PR; CI runs every app's tests; CODEOWNERS pulls each team into review. Deprecation shims only when one PR can't do it. Compat surface is four enumerable seams: auth interface, data-connection interface, log schema, config contract. | Tenant autonomy on upgrade timing. Exit ramp if a team must leave the monorepo: the `workflow_call` check workflow is consumable from any repo. |
| 2 | **Generate, don't clone.** `insights new` copies a template from *inside the SDK package* and substitutes `__NAME__`. Records `scaffold_version` in `platform.toml`. | Cloned templates freeze at clone time and drift invisibly. `scaffold_version` (what generated the app) is the one fact a future `insights upgrade` needs; the SDK version is already determined by the workspace. No template engine — copy and substitute is enough. | CLI is one more thing to maintain. |
